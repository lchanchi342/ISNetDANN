...
from . import globalsZe as globals
#import globalsZe as globals #para hacer el analsissi de LRP heatmaps
import warnings
...
class ISNetFlex(nn.Module):
    ...
    def forward(self,x,runLRPFlex=False,labels=None):
        ...
        if runLRPFlex:
            ...
            if not self.multiple: #multiple False
                #print("multiple: ", self.multiple)
                ...
                if self.selective:
                    #with torch.cuda.amp.autocast(enabled=False):  #linea original
                    with torch.amp.autocast(device_type='cuda', enabled=False): #linea actualizada version pytorch
                ...
                #----NUEVO-----
                #PRUEBA DEL REQUIRE GRAD
                #print("quantity.requires_grad:", quantity.requires_grad)
                ...
                #print(">>> Entrando en modo multi-logit")
                tmp=[]
    ...
    #-----NUEVO------
    def return_features(self, x): #método pass through, que usa el método return_features(x) creado en LRPDensNetZe.py
        """
        Devuelve las features del backbone (sin clasificador).
        Útil para linear eval.
        """
        return self.model.return_features(x)
    ...
    def return_features_backbone(self): #método pass through, que usa el método return_features_backbones() creado en LRPDensNetZe.py
        ...
        """
        return self.model.return_features_backbone()

    @property
    def num_features(self): #para extraer num_features de la clase de LRPDensNetZe.py
        """
        Número de features que salen del backbone antes del clasificador.
        """
...
def getBackbone(architecture,dropout,classes,pretrained=False,VGGRemoveLastMaxpool=False):
    ...
    if (architecture=='densenet121'):
        from . import LRPDenseNetZe as LRPDenseNet
