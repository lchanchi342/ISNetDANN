...
# Linear evaluation utilities for assessing representations.
# Trains simple classifiers (Logistic Regression, Random Forest)
# on learned representations to evaluate encoding of demographic attribute
...
from model_eval.eval_helper import binary_metrics, prob_metrics
...
def get_representations(algorithm, loader, device):
    ...
    """
    Extracts feature representations from a trained model (without the final prediction layer),
    together with labels and sensitive attributes.
    """
    ...
    with torch.no_grad():
        for _, x, y, a in loader:
            z = algorithm.return_feats(x.to(device)).detach().cpu().numpy()  # Returns features (before the final prediction layer)
            zs.append(z) #features
            ys.append(y) #labels
            atts.append(a) #demographic atributtes
...
def fit_model(train_X, train_Y, val_X, val_Y, test_X, test_Y, model_type='lr'):
    ...
    """
    Fits a simple classifier (logistic regression or random forest) on top of representations.
    Hyperparameters are selected using validation data, and performance is evaluated
    on validation and test sets.
    """
    
    if model_type == 'lr': #logistic regresion
        pipe = Pipeline(steps=[
            ('model', LogisticRegression(random_state=42, n_jobs=-1))
        ])
        param_grid = {
            'model__C': 10**np.linspace(-5, 1, 10) # regularization strength
        }
        ...
        param_grid = {
            'model__max_depth': list(range(1, 7))  # tree depth
        }
    else:
    ...
    pds = PredefinedSplit(test_fold=np.concatenate([np.ones((len(train_X),))*-1, np.zeros((len(val_X),))]))
     ...
     # Hyperparameter search using AUROC (one-vs-rest) as the selection metric 
    ...
    # Refit the model with the best hyperparameters on the training set
    pipe = clone(
    ...
    #evaluation on validation (va) and test (te) sets
    for sset, X, Y in zip(['va', 'te'], [val_X, test_X], [val_Y, test_Y]):
        preds = pipe.predict_proba(X) # predicted probabilities
        if len(label_set) == 2:
            preds = preds[:, 1]
            preds_rounded = preds >= 0.5  # threshold at 0.5
        else:
            preds_rounded = preds.argmax(1)# Multiclass: select the class with highest probability

        # Binary classification metrics (accuracy, TPR, FPR, etc.)
        res[sset] = binary_metrics(Y, preds_rounded, label_set=label_set, return_arrays=True)
       
        # Probability-based metrics (BCE, ECE, AUROC, etc.)
...
def eval_lin_attr_pred(train_zs, train_atts, train_ys, val_zs, val_atts, val_ys, test_zs, test_atts, test_ys):
    ...
    """
    Evaluates how well sensitive attributes can be predicted from learned representations.
    """
    
    res = {}
    for model_type in ['lr', 'rf']:
        
        # Unconditional attribute prediction (ignores class label)
        res[f'{model_type}_uncond'] = fit_model(
            train_zs, train_atts, val_zs, val_atts, test_zs, test_atts, model_type=model_type)

        # Conditional on class label = 0
        ...
        # Average metrics across conditional models (y=0 and y=1)
        cond_avg = {'va': {}, 'te': {}}
